...
<body>
	...
	<p align="center">
		<form action="TelaDeCadastroNovoMedico.php" method="POST">
			...
			<br>
			Especialidade:* <input type="text" name="especialidadeInformada" maxlength="20" value="">
			<br>
			CRM:* <input type="text" name="crmInformado" maxlength="20" value="">
			<br>
			Endereço:
			<br>
...
<?php
	...
	$crmInformado= $_POST["crmInformado"];
	...
	*/
	...
	if(($nomeInformado == "") || ($telefoneInformado == "") || ($emailInformado == "") || ($cpfInformado == "") || ($ruaInformada == "") || ($numeroInformado == "") || ($complementoInformado == "") || ($cepInformado == "") || ($bairroInformado == "") || ($paisInformado == "") || ($senhaEscolhida == "") || ($confirmacaoSenhaEscolhida == "") || ($cidadeInformada == "") || ($ufInformada == "") || ($crmInformado == "") || ($especialidadeInformada == ""))
	{
		unset($bd1);
		header("Location: InformeDadosObrigatoriosMedico.html");
	}
	else	//se todos os dados estão deividamente preenchidos
		{
			...
			{
				unset($bd1);
				header("Location: CpfJaCadastradoMedico.html");
			}
			else 
				if($senhaEscolhida != $confirmacaoSenhaEscolhida)		//se a confirmação da senha falhou
				{
					unset($bd1);
					header("Location: SenhaNaoConfereMedico.html");
				}
				else	//se o cpf ainda não existe no sistema e se a senha informada confere continuamos com o cadastro
					{
						...
						*/
						...
						$usuario1->setCodigoTipoUsuario(CODIGO_TIPO_USUARIO_MEDICO);
						...
						$codEndMed= mysql_insert_id();
						
						if($statusQuery4 == 0)
						{
							header("Location: ErroAoCadastrarMedico.html");
						}
						...
						$medico1->setCpf($cpfInformado);
						$medico1->setNomeMedico($nomeInformado);
						$medico1->setEmailMedico($emailInformado);
						$medico1->setTelefone($telefoneInformado);
						$medico1->setCodUsuario($codUsuarioMed);
						$medico1->setCrm($crmInformado);
						$medico1->setEspecialidade($especialidadeInformada);
						$end1->setCodEnd($codEndMed);
						...
						$query6= InsereDadosPessoaisMedico.$medico1->getCpf().", ".$medico1->getNomeMedico().", ".$medico1->getTelefone().", ".$medico1->getEmailMedico().", ".$end1->getCodEnd().", ".$medico1->getCodUsuario().", ".$medico1->getEspecialidade().", ".$medico1->getCrm().")";
						//inserimos através do objeto $bd1
